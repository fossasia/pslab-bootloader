...
static enum BOOT_COMMAND_RESULT EraseFlash(void)
{
    ...
    while (goodErase && (eraseAddress < (pcommand->address + pcommand->dataLength * FLASH_ERASE_PAGE_SIZE_IN_PC_UNITS)))
    {
        if ( IsLegalAddress(eraseAddress) && IsLegalAddress(eraseAddress+FLASH_ERASE_PAGE_SIZE_IN_PC_UNITS-1) )
        {
    ...
    if ((goodErase) && (eraseAddress == (pcommand->address + pcommand->dataLength * FLASH_ERASE_PAGE_SIZE_IN_PC_UNITS)))
    {
...
static enum BOOT_COMMAND_RESULT ReadFlash(void)
{
   ...
   (void)BOOT_COM_Read(commandArray, sizeof(struct CMD_STRUCT_0) );
    ...
    if ( IsLegalAddress(pCommand->address) && IsLegalAddress(pCommand->address+pCommand->dataLength-1) && 
